...
if uploaded_file:
    ...
    if "Вероятность_оттока" not in df.columns:
        ...
        # === Фильтр по сегменту ===
        segment = st.selectbox("Выберите сегмент риска", ["Все", "Надёжный", "Группа риска", "Высокий риск"])
        ...
        st.dataframe(df.sort_values("Вероятность_оттока", ascending=False).head(15))
else:
