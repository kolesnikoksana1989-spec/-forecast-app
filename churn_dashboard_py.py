...
with tab2:
    ...
    if uploaded_file:
        # 🔹 вместо повторного чтения — используем уже загруженный df
        # перемещаем «указатель» файла в начало, если нужно
        uploaded_file.seek(0)

        # теперь можно снова считать
        df = pd.read_csv(uploaded_file)
        ...
        if "Вероятность_оттока" not in df.columns:
            ...
            st.dataframe(
                df[["Клиент_ID", "Средний_чек", "Количество_покупок", "Вероятность_оттока", "Рекомендация"]]
